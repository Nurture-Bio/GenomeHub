...
## VII. The Prophecy and the Void

*On Optimistic Projection and the Amber Predicate*

### The Koan

The novice built a histogram that predicted the future. "Master, the bars move instantly when I drag! But some bars appear in the amber void, where the Water has already been drained by other filters."

The Master studied the amber glow. "You are prophesying where the Water might be, but you have forgotten where you already *know* it is not. A prophecy that contradicts known evidence is not optimism—it is a lie."

### The Physics

When the Hand moves a slider, the server needs 80–200ms to compute the real constrained histogram. During that gap, the Glass must show *something*. The `projectHistogram` function rescales the static (unfiltered) distribution within the thumb range—a local prophecy that requires zero network.

$$
\text{projected}[i] = \frac{\text{static}[i]}{\max(\text{static}[lo..hi])} \times \max(\text{static})
$$

When the server responds, bars that cross-filtering excluded "fall off." That delta—the difference between the prophecy and the truth—is the science the researcher came to see.

But the prophecy must respect what is already known. Two laws govern correctness.

### The First Law: The State Machine

A single `SliderPhase` value with explicit transitions replaces boolean flag intersections. The old regime—`isDragging × isPanning × justDropped × wasActor`—produced emergent states that no one could audit.
...
Three mutually exclusive derived booleans: `isActor`, `isSpectator`, `settled`. Every rendering decision branches from these—nothing else.

**The Flash Killer:** `effectivePhase` is an inline derivation that detects `querying && !pending → idle` on the same render frame:

```typescript
const effectivePhase: SliderPhase = phase === 'querying' && !pending ? 'idle' : phase;
```

The `useEffect` that sets `phase = 'idle'` is bookkeeping only. The visual is already correct on the same render that `pending` goes false. No gap, no amber flash.

### The Second Law: The Amber Predicate

The prophecy must be optimistic when expanding into unknown territory, but must show nothing in a confirmed cross-filter void.

**The trap:** `activeConMin` is the data extent from the last query, which includes this slider's own range. The condition `low < activeConMin` fires both when dragging into the cross-filter void AND when dragging past your own old position into territory with real static data. It cannot distinguish these.

**The law:** Check whether amber was visible at **pointerDown**, not whether the thumb is currently past the data extent. `dragStartRef.current` captures the thumb position at the moment the Hand grasped the glass:
...
| The Hand's Intent | Amber at Grab | Prophecy |
|---|---|---|
| Expanding into unknown territory | Not visible | Fully optimistic—bars grow from static distribution |
| Dragging through the void | Visible | Clips at the data boundary—no bars in the void |
| Dragging through true emptiness | n/a | Zero—`staticBins[i] === 0` everywhere |

The histogram and the amber track now speak the same truth from their respective languages: `staticBins[i]` encodes true emptiness, `activeConMin`/`activeConMax` encodes cross-filter constraint, and `dragStartRef` disambiguates which applies at the moment of interaction.

### The Compositor Isolation

DistributionPlot animates 64 SVG `<rect>` elements. SVG attribute transitions (`y`, `height`) are main-thread-interpolated—64 elements × 300ms means the browser recomputes layout every frame. If the user drags a range slider during that 300ms, `syncTrack()` writes CSS variables on the same thread. Two animation systems fighting for the same 16ms budget.

The fix: replace `y`/`height` attribute animation with `transform: scaleY()` from a fixed baseline. `transform` is compositor-composited—GPU thread, zero main-thread cost:
...
Histogram morphing runs on the GPU thread. Range slider drag runs on the main thread. They never collide.

### What the Glass Taught Us

The description of the projection took one paragraph. The implementation took three hours. The cost was paid in two categories:

1. **Boolean flags instead of a state machine.** Four flags with different clearing conditions. A `useEffect` that cleared `wasActor` when `!pending` would self-destruct during the 80ms gap between pointerUp and `START_QUERY` dispatch—`!pending && wasActor` evaluated to true before pending ever arrived. Every fix to one flag's timing broke another flag's invariant. The state machine eliminated the category.

2. **Wrong predicate for clipping.** `activeConMin` conflates this slider's own previous range with cross-filter effects. It cannot answer "is this the cross-filter void or unexplored territory?" The answer was already in the codebase: `dragStartRef.current`. Whether amber was visible at the moment the Hand grasped the glass—not whether the thumb is currently past the data edge—is the correct predicate.
