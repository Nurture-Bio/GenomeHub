...
export default function FileDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-4 p-2 md:p-5 animate-page-enter">
        ...
        <div className="flex flex-col gap-2">
          <div className="mt-0.5">
            <InlineInput
              ...
              onCommit={(v) => {
                if (fileId) updateFile(fileId, { description: v || null });
              }}
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <ChipEditor
              ...
              onAdd={(id) => {
                if (fileId) addFileOrganism(fileId, id);
              }}
              onRemove={(id) => {
                if (fileId) removeFileOrganism(fileId, id);
              }}
              renderPicker={(p) => (
          ...
          <div className="flex items-center gap-2">
            ...
            <ChipEditor
              ...
              onAdd={(id) => {
                if (fileId) updateFile(fileId, { types: [...file.types, id] });
              }}
              onRemove={(id) => {
                if (fileId) updateFile(fileId, { types: file.types.filter((t) => t !== id) });
              }}
              renderPicker={(p) => (
          ...
          {file.tags.length > 0 && (
            <div className="flex gap-0.5 flex-wrap">
              {file.tags.map((t) => (
                <Badge key={t} variant="count" color="dim">
                  {t}
                </Badge>
              ))}
            </div>
      ...
      {file && (
        <div>
          <Text variant="muted" className="mb-1.5 block">
            Collections
          </Text>
          <ChipEditor
            ...
            renderPicker={(p) => (
              <CollectionPicker {...p} variant="surface" size="sm" className="w-44" />
            )}
            renderLabel={(item) => (
              <Link
          ...
          <div>
            ...
            {addingProv && (
              <div className="border border-line rounded-md p-2.5 mb-2 bg-raised">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  />
                  <span
                    className={`transition-opacity duration-fast ${provTargetId ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                  >
                    <button
            ...
            {fileDetail && fileDetail.provenance.upstream.length > 0 && (
              <div className="mb-2">
                ...
                <div className="flex flex-col gap-1">
                  {fileDetail.provenance.upstream.map(
                    (p) =>
                      p.file && (
            ...
            {fileDetail && fileDetail.provenance.downstream.length > 0 && (
              <div className="mb-2">
                ...
                <div className="flex flex-col gap-1">
                  {fileDetail.provenance.downstream.map(
                    (p) =>
                      p.file && (
                        <Card key={p.edgeId} className="p-2 flex items-center gap-2 group">
                          <RelationLabel relation={p.relation} />
            ...
            {fileDetail &&
              fileDetail.provenance.upstream.length === 0 &&
              fileDetail.provenance.downstream.length === 0 &&
              !addingProv && (
                <Text variant="dim" className="animate-fade-up">
                  No data links.
                </Text>
              )}
          </div>
