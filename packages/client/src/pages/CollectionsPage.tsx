  ...
  return (
    <div className="flex flex-col gap-3 md:gap-4 p-2 md:p-5 h-full min-h-0 animate-page-enter">
      <div className="shrink-0">
        ...
        <Text variant="dim">
          {data ? (
            `${data.length} collection${data.length !== 1 ? 's' : ''}`
          ) : isError ? (
            '—'
          ) : (
            <span className="skeleton h-[1lh] w-20 inline-block align-middle rounded-sm" />
          )}
        </Text>
      </div>
        ...
        <table className="w-full border-collapse text-left table-fixed">
          <thead className="sticky top-0 bg-surface-raised z-sticky">
            <tr className="border-b border-border">
              ...
              <th className="tbl-cell w-36">
                <Text variant="muted">Technique</Text>
              </th>
              <th className="tbl-cell w-40">
                <Text variant="muted">Organism</Text>
              </th>
              <th className="tbl-cell w-32">
                <Text variant="muted">Type</Text>
              </th>
              <th className="tbl-cell w-16 text-right">
                  ...
                  >
                    <td className="tbl-cell">
                      <Link to={`/collections/${c.id}`} className="no-underline">
                        ...
                        {c.description && (
                          <Text variant="dim" className="truncate max-w-xs block">
                            {c.description}
                          </Text>
                        )}
                      </Link>
                    ...
                    <td className="tbl-cell overflow-hidden">
                      <ChipEditor
                        ...
                        renderPicker={(p) => (
                          <TechniquePicker {...p} variant="surface" size="sm" className="w-32" />
                        )}
                        maxVisible={2}
                      />
                    </td>
                    <td className="tbl-cell overflow-hidden">
                      <ChipEditor
                        ...
                        renderPicker={(p) => (
                          <OrganismPicker {...p} variant="surface" size="sm" className="w-36" />
                        )}
                        maxVisible={2}
                      />
                    </td>
                    <td className="tbl-cell overflow-hidden">
                      <ChipEditor
                        ...
                        onRemove={(id) =>
                          updateCollection(c.id, { types: c.types.filter((t) => t !== id) })
                        }
                        renderPicker={(p) => (
            ...
            <Link key={c.id} to={`/collections/${c.id}`} className="no-underline">
              <Card className="p-2.5 flex flex-col gap-1 hover:border-interactive transition-colors duration-fast cursor-pointer">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Text variant="body" className="truncate flex-1 min-w-0">
                    {c.name}
                  </Text>
                  {c.types.map((t) => (
                ...
                {c.description && (
                  <Text variant="dim" className="truncate">
                    {c.description}
                  </Text>
                )}
                <div className="flex items-center gap-2 flex-wrap">
