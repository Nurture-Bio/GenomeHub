...
export default function ComboBox({
  ...
  const recentItems = recentIds ? items.filter((i) => recentSet.has(i.id) && i.id !== value) : [];
  // Exclude recent items from the main list so cmdk never sees duplicate values
  const mainItems =
    recentItems.length > 0 ? items.filter((i) => !recentSet.has(i.id) || i.id === value) : items;
    ...
    >
      ...
      {(!item.icon || item.description) && (
        <div className="flex-1 min-w-0">
          ...
          {item.description && (
            <div className={cx(text({ variant: 'dim' }), 'text-body truncate')}>
              {item.description}
            </div>
          )}
        </div>
