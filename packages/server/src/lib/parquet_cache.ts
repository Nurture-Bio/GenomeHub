/**
 * Ephemeral Parquet cache — downloads S3 files to local disk once,
 * serves all subsequent DuckDB queries from NVMe-speed local I/O.
 *
 * - Mutex: concurrent requests for the same key share one download promise.
...
/**
 * Resolve a Parquet S3 key to a local filesystem path.
 * Downloads from S3 on first access; returns cached path thereafter.
 *
...
export async function resolveLocalParquet(s3Key: string): Promise<string> {
  ...
  // Mutex: if another request is already downloading this key, wait for it
