...
function profileEngineResult(file: GenomicFile): void {
  ...
  fileRepo
    .update(file.id, { parquetStatus: 'converting' })
      ...
      try {
        ...
        const isConnRefused =
          ...
          (pollErr instanceof Error &&
            pollErr.cause &&
            (pollErr.cause as NodeJS.ErrnoException).code === 'ECONNREFUSED');
...
router.delete(
  ...
  asyncWrap(async (req, res) => {
    ...
    if (job.engineUrl && job.engineJobId) {
      fetch(`${job.engineUrl}/api/jobs/${job.engineJobId}`, { method: 'DELETE' }).catch(() => {
        /* best-effort */
      });
    }
  ...
  asyncWrap(async (req, res) => {
    ...
    if (methodRes.status === 202) {
      ...
      runAsyncJob(
        ...
        /* error already captured in job.error */
      });
