  ...
  asyncWrap(async (req, res) => {
    ...
    if (organismId) {
      const orgEdges = await edgeRepo.find({
        where: {
          sourceType: 'collection' as any,
          targetType: 'organism' as any,
          targetId: organismId,
          relation: 'targets' as any,
        },
      });
    ...
    const fileCounts = await edgeRepo
      ...
      .where(
        "e.source_type = 'file' AND e.target_type = 'collection' AND e.relation = 'belongs_to'",
      )
  ...
  asyncWrap(async (req, res) => {
    ...
    const fileEdges = await edgeRepo.find({
      where: {
        ...
        targetType: 'collection' as any,
        targetId: col.id,
        relation: 'belongs_to' as any,
      },
    });
    ...
    // External links
    const linkEdges = await edgeRepo
      .createQueryBuilder('e')
...
router.post(
  ...
  asyncWrap(async (req, res) => {
    ...
    if (!techniqueId) {
      res.status(400).json({ error: 'techniqueId required' });
      return;
    }

    const technique = await AppDataSource.getRepository(Technique).findOneBy({ id: techniqueId });
    if (!technique) {
      res.status(400).json({ error: 'technique not found' });
      return;
    }
...
router.delete(
  ...
  asyncWrap(async (req, res) => {
    ...
    if (fileCount > 0) {
      res.status(409).json({
        error: `Cannot delete: collection still has ${fileCount} reference(s). Remove all files first.`,
      });
      return;
    }
