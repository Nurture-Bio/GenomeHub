...
__export(core_exports, {
  ...
  CELL_H_PADDING: () => CELL_H_PADDING,
  VirtualChamber: () => VirtualChamber,
  asBatchSeq: () => asBatchSeq,
  asMs: () => asMs,
  asPixelSize: () => asPixelSize,
  asPoolSlot: () => asPoolSlot,
  asRowIndex: () => asRowIndex,
  createRecordBatchStream: () => createRecordBatchStream,
  encodeRecordBatch: () => encodeRecordBatch,
  useAtomicSlice: () => useAtomicSlice,
  useStabilityOrchestrator: () => useStabilityOrchestrator
});
...
function useAtomicSlice(store, selector, equalityFn = Object.is) {
  ...
  const equalityRef = (0, import_react.useRef)(equalityFn);
  selectorRef.current = selector;
  equalityRef.current = equalityFn;
  const cachedSlice = (0, import_react.useRef)(null);
...
function useStabilityOrchestrator(options) {
  ...
  const [store] = (0, import_react2.useState)(() => new AtomicStore());
  const workerRef = (0, import_react2.useRef)(null);
  const containerRef_internal = (0, import_react2.useRef)(null);
  const layoutRef = (0, import_react2.useRef)(null);
  const seqRef = (0, import_react2.useRef)(asBatchSeq(0));
  const roRef = (0, import_react2.useRef)(null);
  const schemaRef = (0, import_react2.useRef)(schema);
  schemaRef.current = schema;
  const ackResolversRef = (0, import_react2.useRef)(/* @__PURE__ */ new Map());
  (0, import_react2.useEffect)(() => {
    const worker = new Worker(
      new URL("./data-worker.js", import_meta.url),
      { type: "module" }
    );
  ...
  const scrollToRow = (0, import_react2.useCallback)((row) => {
    const el = containerRef_internal.current;
    const lay = layoutRef.current;
    if (!el || !lay) return;
